...
z_dot = -10;    %%注意这个z_dot这个量，必须要保证一阶差分的初值是没问题的，才可以保证最终的结果，那么之前一直发散，和这个值有很大关系
zL_70m_fdatool = 0;
...
subplot(2,1,2);plot(z_save);title('高低不平顺');xlabel('采样点 /0.25m');ylabel('int');set(gca,'Fontname','Times New Roman','fontsize',14);
figure1 = figure('Color',[1 1 1]);plot(yL_ref70m(start:start+len-1));hold on;plot(yL_ref30m(start:start+len-1));set(gca,'Fontname','Times New Roman','fontsize',14);grid on;
