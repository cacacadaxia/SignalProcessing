...
%     dtemp = (dtemp + dincl * w_bt_dot /  (tbs/1e5))/246083.654 /(0.016932*9.8) * 3276.8 * 1.0057; %%1.0057是前后两个scali的参数比值
    dtemp = (dtemp + dincl * w_bt_dot /  (tbs/1e5)) / 246083.654  * 1.0057; %%1.0057是前后两个scali的参数比值
...
%     1/(0.016932*9.8) * 3276.8 = 19748
    ...
    infp = B2(gpin(i)/s1,tbs);   %%这个B多次使用所以产生影响
    infp_save(i,1) = infp;
    inc = infp + dtmp_Fz;     
...
figure;plot(lfcrp * 19748);hold on;plot(lfcrp_comp);legend matlab gj;
figure;plot(lfcrp_comp - lfcrp*19748);


%%
% dt = (gpvro - gpvlo) / (32768/127);
% sita_bt_tp = dt / 1435;
% figure;plot(sita_bt_tp/pi*180);
