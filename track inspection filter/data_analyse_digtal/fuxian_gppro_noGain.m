...
% 10.15
%        2.不用调整增益
%        3. 问题是：如果不用管增益的话，最终的合成值肯定是错误的，但是这个增益带来
%           数值问题，不能随便加上去
%--------------------------------------------------------------------------
...
gypitch = gypitch/246083.654;
gppl = gppl/13852;
TBS = TBS/1e5;
gpvlo = gpvlo/258.016;
gpvro = gpvro/258.016;
Guage = 59.5 * 25.4;
sita_bt = (gpvro - gpvlo) ./ Guage;
sita_b = sita_b / 1638.4  / 180 * pi;
...
for i = 1:length(gppl)
    az_Gz(i,1) = G( gppl(i) * 13852 , TBS(i) * 1e5 );%%调整一下Gz，这应该就行了吧？
    az_Gz(i,1) = az_Gz(i,1) / 13852 * 1e3;   %%mm/s^2
end

% az_Gz = quzheng( az_Gz );
% amarm = sita_b_dot2.*ararm_par;
amarm = sita_b_dot2 / 2 .* Guage;
% pgrav = sita_b.^2.*TBS.^2*pgscale;
pgrav = sita_b.^2 .* TBS.^2 * 9.8 * 1e3;
...
ratio = 1;figure;plot(ratio.*zL_70m);hold on;plot(pmcol_ref70m(177:end));legend matlab gj
